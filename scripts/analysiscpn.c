...
double chixisq(double q2, double g0, double gp, double temp1, int L){
	return (q2/(L*L))*(1/(4*sin(M_PI/L)*sin(M_PI/L)))*(g0/gp - 1);
	}
...
int main(void){
	FILE *data, *result, *resfss;
	int i, j, k, l, r, M, temp1, seed, nmeas, nwords, dim, L[D], term, err;
	double *g0, *gp, *mu2, *qu, Chixisq, dChixisq, G0, dG0, Gp, Qu, dQu, Q2, Q4, Xi, dXi, Chi, dChi, B2, dB2, U, dU, Mu2, Mu2sq, beta, lambda, temp2, *ener, E, dE, naive, autocorr, *rplaq, *iplaq, RPlaq, IPlaq, dRPlaq, dIPlaq;
	char ifname[100], ofname[100], stemp[100], fssname[100];
	err = scanf("%s\n", stemp);
	if(err != 1){//file path
	...
	if(scanf("%d	%d\n", &nmeas, &nwords) != 2){//rows and columns
		fprintf(stderr, "Error in file reading, (%s, %d)\n", __FILE__, __LINE__);
		exit(EXIT_FAILURE);
		}
	...
	if(scanf("%d	%lf	%lf\n", &L[0], &beta, &lambda) != 3){//L, beta and lambda parameters
		fprintf(stderr, "Error in file reading, (%s, %d)\n", __FILE__, __LINE__);
		exit(EXIT_FAILURE);
		}
	if(((nmeas*6) != nwords)){
	...
	//snprintf(ofname, sizeof(fssname), "res_cont_%.3f.txt", lambda);
	...
	//use the fss code for the plaquette case in cp9 if necessary
	//snprintf(fssname, sizeof(fssname), "res_cont_%.3f.txt", lambda);
	...
	naive = 0;
	for(i=0; i<nmeas; i++){
		naive += pow(Chi - ((double)(qu[i]*qu[i])/dim), 2);
		}
	naive = naive/nmeas/(nmeas-1);
	naive = sqrt(naive);
	autocorr = (10/2)*((dChi*dChi/naive/naive) - 1);
	printf("Topological susceptibility correlation time = %f\n", autocorr);
	*/
