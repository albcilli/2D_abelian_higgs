...
void measure(int dim, double beta, double lambda, int L[D], double complex **scalfld, double complex **gaugefld, int **next, FILE *ofp){
	...
	//ener = energytest(dim, scalfld, gaugefld, next);//added for test
	
	err = fprintf(ofp, "%f	%f	%f	%f	%f	%f\n", g0, gp, mu2, Qu, creal(Plaq), cimag(Plaq));//printing results on the file pointed by ofp
	if(err < 0){
...
int main(void){
	...
	double complex **field, **gauge, microtests[N], microtestg[D];//pointer for dim*N and dim*D arrays for scalar field and gauge field
